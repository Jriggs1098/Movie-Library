 ...
 * 
 * This script provides autosuggest when adding movies to the database.
 */
var xmlHttp;
var numTitles = 0;  //total number of suggested movies titles
var activeTitle = -1;  //movie title currently being selected
var titleAddBoxObj, suggestAddBoxObj, dataType;
...
window.onload = function () {
    ...
    //DOM objects
    titleAddBoxObj = document.getElementById('title-add');
    suggestAddBoxObj = document.getElementById('suggest-div');
    dataType = document.getElementById('type').value;
};

window.onclick = function () {
    suggestAddBoxObj.style.display = 'none';
};
...
function displayTitles(titles) {
    ...
    suggestAddBoxObj.innerHTML = divContent;
    suggestAddBoxObj.style.display = 'block';
}
...
function handleKeyUp(e) {
    ...
    if (e.keyCode === 38 && activeTitle > 0) {
        ...
        activeTitleObj.style.backgroundColor = "#a0a9bc";
        activeTitle--;
        activeTitleObj = document.getElementById("s_" + activeTitle);
        activeTitleObj.style.backgroundColor = "#d5d9f2";
        titleAddBoxObj.value = activeTitleObj.innerHTML;
        return;
    }
...
function clickTitle(title) {
    ...
    titleAddBoxObj.value = title.innerHTML;
